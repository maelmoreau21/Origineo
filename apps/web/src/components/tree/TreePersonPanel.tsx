...
function fmt(p:any){
  return {
    ...
    title:p.title??null,
    baptismDate:p.baptismDate??p.baptism_date??null,
    baptismPlace:p.baptismPlace??p.baptism_place??null,
    burialDate:p.burialDate??p.burial_date??null,
    burialPlace:p.burialPlace??p.burial_place??null,
    deathCause:p.deathCause??p.death_cause??null,
    religion:p.religion??null,
    physicalDescription:p.physicalDescription??p.physical_description??null,
...
export default function TreePersonPanel({person,personId,isAdmin,onClose,onCenterOnPerson,onPersonUpdated,token}:TreePersonPanelProps){
  ...
  const [childrenFromApi,setChildrenFromApi]=useState<any[]>([]);
  ...
  useEffect(()=>{
    ...
    // Fetch relationships to get children
    relationshipApi.getByPerson(personId).then(r => {
      const kids = Array.isArray(r.data?.asParent) ? r.data.asParent : [];
      setChildrenFromApi(kids);
    }).catch(() => setChildrenFromApi([]));
  ...
  useEffect(()=>{
    if(p){
      setEditForm({
        ...
        nickname:p.nickname||'',title:p.title||'',
        gender:p.gender,
        birthDate:p.birthDate?new Date(p.birthDate).toISOString().slice(0,10):'',
        birthPlace:p.birthPlace||'',
        baptismDate:p.baptismDate?new Date(p.baptismDate).toISOString().slice(0,10):'',
        baptismPlace:p.baptismPlace||'',
        deathDate:p.deathDate?new Date(p.deathDate).toISOString().slice(0,10):'',
        deathPlace:p.deathPlace||'',
        burialDate:p.burialDate?new Date(p.burialDate).toISOString().slice(0,10):'',
  ...
  const handleSave=useCallback(async()=>{
    ...
    try{
      await personApi.update(personId,{
        ...
        baptismDate:editForm.baptismDate||undefined,
        baptismPlace:editForm.baptismPlace.trim()||undefined,
        deathDate:editForm.deathDate||undefined,
        deathPlace:editForm.deathPlace.trim()||undefined,
        burialDate:editForm.burialDate||undefined,
        burialPlace:editForm.burialPlace.trim()||undefined,
        deathCause:editForm.deathCause.trim()||undefined,
  ...
  return(<>
    <aside className="tpp">
      <div className="tpp-inner">
        ...
        <div className="tpp-content">
            ...
            <div className="tpp-info">
              {!editing?(
                <div className="tpp-fields">
                  <div className="tpp-sub-title">Identité</div>
                  ...
                  {p.religion&&<Field label="Religion" value={p.religion}/>}
                  {p.physicalDescription&&<Field label="Description physique" value={p.physicalDescription}/>}
                  {p.education&&<Field label="Éducation" value={p.education}/>}
                  {p.professions.length>0&&<Field label="Professions" value={p.professions.join(', ')}/>}
                  {p.residences.length>0&&<Field label="Résidences" value={p.residences.join(', ')}/>}
                  
                  {p.notes&&<div style={{marginTop:12}}><Field label="Notes" value={p.notes}/></div>}
                ...
                <div className="tpp-edit-form">
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                    <EditField label="Surnom" value={editForm.nickname} onChange={v=>setEditForm({...editForm,nickname:v})}/>
                    <EditField label="Titre" value={editForm.title} onChange={v=>setEditForm({...editForm,title:v})}/>
                  </div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                    <EditField label="Date baptême" value={editForm.baptismDate} onChange={v=>setEditForm({...editForm,baptismDate:v})} type="date"/>
                    <EditField label="Lieu baptême" value={editForm.baptismPlace} onChange={v=>setEditForm({...editForm,baptismPlace:v})}/>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                    <EditField label="Date inhumation" value={editForm.burialDate} onChange={v=>setEditForm({...editForm,burialDate:v})} type="date"/>
                    <EditField label="Lieu inhumation" value={editForm.burialPlace} onChange={v=>setEditForm({...editForm,burialPlace:v})}/>
                  </div>
                  <EditField label="Cause du décès" value={editForm.deathCause} onChange={v=>setEditForm({...editForm,deathCause:v})}/>

                  <div className="tpp-sub-title" style={{marginTop:12}}>Détails personnels</div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                    <EditField label="Nationalité" value={editForm.nationality} onChange={v=>setEditForm({...editForm,nationality:v})}/>
                    <EditField label="Religion" value={editForm.religion} onChange={v=>setEditForm({...editForm,religion:v})}/>
                  </div>
                  <EditField label="Éducation / Diplômes" value={editForm.education} onChange={v=>setEditForm({...editForm,education:v})}/>
                  <div className="tpp-edit-field">
                    <label>Description physique</label>
                    <textarea value={editForm.physicalDescription} onChange={e=>setEditForm({...editForm,physicalDescription:e.target.value})} rows={2}/>
                  </div>
                  <div className="tpp-edit-field">
                    <label>Professions (1 par ligne)</label>
                    <textarea value={editForm.professions} onChange={e=>setEditForm({...editForm,professions:e.target.value})} rows={3}/>
                  </div>
                  <div className="tpp-edit-field">
                    <label>Résidences (1 par ligne)</label>
                    <textarea value={editForm.residences} onChange={e=>setEditForm({...editForm,residences:e.target.value})} rows={3}/>
                  </div>

                  <div className="tpp-sub-title" style={{marginTop:12}}>Notes</div>
                  <div className="tpp-edit-field">
          ...
          {tab==='FAMILY'&&(
            <div className="tpp-family">
              <div className="tpp-sub-title">Famille & Unions</div>
              {unions.length===0 && childrenFromApi.length===0 && <div className="tpp-empty">Aucune union ou enfant enregistré.</div>}
              
              {unions.map((u:any)=>{
                const partner=u.partner1Id===personId?u.partner2:u.partner1;
                // Group children locally: A child of this union is someone who has this person AND the partner as parents.
                // Since we don't have the full graph here, we just display "Les enfants avec [Partenaire] ne sont pas explicitement liés à l'union dans cette vue simplifiée" unless we can fetch them.
                // Wait, we have childrenFromApi. We don't have the child's *other* parent ID directly in the child object. 
                // We'll just display the union for now.
                return(
              ...
              {childrenFromApi.length>0&&(
                <div style={{marginTop:16}}>
                  <div className="tpp-sub-title">Enfants connus ({childrenFromApi.length})</div>
                  <div className="tpp-child-list">
                    {childrenFromApi.map((rel:any)=>(
                      <div key={rel.id} className="tpp-child-card">
                        👶 {dn(rel.child)}
                      </div>
                    ))}
                  </div>
    ...
    <style>{`
      ...
      .tpp-tabs{display:grid;grid-template-columns:1fr 1fr 1fr;border-bottom:1px solid hsla(220,20%,28%,0.3);}
      .tpp-tab{padding:10px 4px;border:none;background:transparent;color:var(--color-text-muted);font-size:0.7rem;font-weight:600;cursor:pointer;transition:all 150ms ease;border-bottom:2px solid transparent;}
      ...
      .tpp-field{flex:1;padding:6px 10px;border-radius:8px;background:hsla(220,20%,16%,0.5);border:1px solid hsla(220,20%,28%,0.2);}
      ...
      .tpp-field-value{font-size:0.78rem;color:var(--color-text-primary);margin-top:2px;white-space:pre-wrap;}
      .tpp-sub-section{margin-top:8px;}
      .tpp-sub-title{font-size:0.7rem;font-weight:700;color:var(--color-text-primary);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid hsla(220,20%,28%,0.3);padding-bottom:4px;}
      
      /* Family tab */
      .tpp-family{display:flex;flex-direction:column;gap:8px;}
      .tpp-union-block{padding:8px 10px;border-radius:8px;background:hsla(220,20%,16%,0.5);border:1px solid hsla(220,20%,28%,0.3);margin-bottom:8px;}
      .tpp-union-header{display:flex;justify-content:space-between;align-items:center;font-size:0.78rem;font-weight:600;color:var(--color-text-primary);}
      .tpp-child-list{display:grid;gap:6px;}
      .tpp-child-card{padding:6px 10px;border-radius:6px;background:hsla(220,20%,14%,0.4);border:1px solid hsla(220,20%,28%,0.15);font-size:0.74rem;}
      ...
      .tpp-admin-actions{display:flex;gap:6px;margin-top:16px;}
      .tpp-edit-form{display:flex;flex-direction:column;gap:8px;}
      .tpp-edit-field{display:flex;flex-direction:column;gap:3px;flex:1;}
      .tpp-edit-field label{font-size:0.62rem;color:var(--color-text-muted);font-weight:600;text-transform:uppercase;}
      .tpp-edit-field input,.tpp-edit-field select,.tpp-edit-field textarea{padding:7px 10px;border-radius:7px;border:1px solid hsla(220,20%,28%,0.5);background:hsla(220,20%,12%,0.7);color:var(--color-text-primary);font-size:0.76rem;outline:none;font-family:inherit;resize:vertical;width:100%;}
      .tpp-edit-field input:focus,.tpp-edit-field select:focus,.tpp-edit-field textarea:focus{border-color:hsl(200,80%,50%);}
      .tpp-edit-actions{display:flex;gap:6px;justify-content:flex-end;margin-top:12px;}
      ...
      .tpp-empty{font-size:0.72rem;color:var(--color-text-muted);padding:8px 0;font-style:italic;}
