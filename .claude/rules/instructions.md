...
- **Interaction arbre** : clic simple sur une carte = selection et mise a jour de `PersonInspector`; double-clic = recentrer/recharger l'arbre sur cette personne. La fiche complete reste accessible par le bouton `Fiche complete` de l'inspecteur. Le centrage utilise un repere canvas `left: 0; top: 0`, calcule la position reelle du noeud dans le viewport et les controles zoom affichent `-`, pourcentage, `+`, `Ajuster`.
...
- **Liens enfants/fratrie** : un enfant avec deux parents reels doit toujours etre rattache au midpoint du couple correspondant, meme si l'union est inferee ou si le second parent visible n'est pas le conjoint local du noeud courant. Ne jamais degrader un frere/une soeur a un lien parent-seul lorsque deux parents sont connus et visibles.
...
- **Chemins SVG** : les liens parent/enfant doivent rester orthogonaux avec un rayon d'angle constant. Eviter les courbes D3 variables (`curveBasis`) pour les liens familiaux; utiliser des bus par union legerement decales quand plusieurs unions partent du meme parent afin de limiter les superpositions.
...
- **Layout visuel** : moteur SVG maison inspire de `donatso/family-chart` (MIT) : conjoints alignes, enfants groupes par union, bus parent-enfant depuis le midpoint du couple, coparents inferes si l'union manque, chemins orthogonaux arrondis, fenetre navigable.
- **Interaction workspace** : clic simple selectionne et rafraichit l'inspecteur; double-clic recentre/recharge l'arbre sur la personne; la fiche profil reste accessible depuis `Fiche complete` dans l'inspecteur.
